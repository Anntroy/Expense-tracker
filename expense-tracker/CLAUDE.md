...
- **Dónde poner la lógica para poder probarla:** no hay tests de componentes (ni `jsdom`): lo que decide algo en la interfaz se extrae a una función pura en `src/lib/` y se prueba ahí; el componente solo la llama. Ejemplos: `monthTotals` (totales del mes: el ahorro resta del balance y los desactivados no cuentan), `viewAfterAdd` (mes y filtro de persona tras agregar), `resolveMemberChoice` (miembro por defecto del formulario), `windowEndShowing`/`shortcutRange`/`monthOfDate` (fechas), `suggestedCategories`/`mergeCategories`, `wrongPinMessage`. Al añadir comportamiento nuevo en un componente, seguir el mismo criterio.
...
## Empaquetado e instaladores (electron-builder)

- Config en el bloque `"build"` de `package.json` (no un `electron-builder.yml` aparte): `appId`, `productName`, `directories.output` (`release/`, donde caen los instaladores) y `directories.buildResources` (`resources/`, **no** `build/` — ese nombre ya está en `.gitignore` para otra cosa y el ícono quedaría sin commitear si se usara).
- `files` solo lista `dist-electron/**/*` y `out/**/*`: `package.json` y los `node_modules` de producción se incluyen siempre aparte, así que alcanza con nombrar el código propio compilado.
- `resources/icon.svg`: ícono placeholder (mismo azul que `--saving`), un solo SVG de 1024×1024 a partir del cual `electron-builder` genera `.icns` (mac), `.ico` (Windows) y los PNG de Linux — no hace falta exportar cada formato a mano. Conviene reemplazarlo por un ícono de verdad antes de distribuir la app fuera de uso personal.
- `asarUnpack: ["node_modules/better-sqlite3/**/*"]`: el módulo nativo no puede cargarse desde dentro del `.asar` (no soporta `dlopen`), así que queda sin comprimir al lado.
- **Targets por plataforma:** mac → `dmg` (`x64` y `arm64` por separado, no `universal`, para no depender de que `better-sqlite3` tenga binario universal); Windows → `nsis` (`x64`, instalador asistido con `oneClick: false` para poder elegir carpeta); Linux → `AppImage` y `deb` (`x64`). Scripts nuevos `npm run build:mac` / `build:win` / `build:linux` (pasan `--mac`/`--win`/`--linux` a `electron-builder`; `npm run build` sin flag usa la plataforma actual).
- **`next`, `react`, `react-dom` y `recharts` están en `devDependencies`, no en `dependencies`:** solo hacen falta para `next build` (que los deja ya compilados/inlineados en `out/`), no en tiempo de ejecución del proceso principal de Electron. Como `electron-builder` siempre empaqueta los `node_modules` de producción enteros, tenerlos en `dependencies` inflaba el instalador sin necesidad. `better-sqlite3`, `drizzle-orm` y `zod` sí quedan en `dependencies`: los usa `electron/` en tiempo de ejecución.
- **No se puede cross-compilar el módulo nativo:** generar el instalador de Windows o Linux **desde macOS falla** — verificado (`npm run build:linux` en Mac tira `node-gyp does not support cross-compiling native modules from source`). Cada instalador hay que generarlo en su propio sistema operativo (o con CI, ej. GitHub Actions con matriz `macos-latest`/`windows-latest`/`ubuntu-latest`). El de mac sí se generó y probó localmente (`release/*.dmg`, sin firmar — sin un Apple Developer ID, macOS avisa con Gatekeeper al abrirlo en otra máquina; para distribuir de verdad hace falta firma y notarización, no configuradas).
- Ojo: la ruta del proyecto tiene un espacio (`Claude code`) y eso dispara un warning de `node-gyp` en cada rebuild nativo (`nodejs/node-gyp#65`). Hoy no rompe nada porque `better-sqlite3` usa un binario prebuilto, pero si alguna vez hiciera falta compilar desde fuente para alguna combinación de arquitectura/versión de Electron sin prebuilt, ahí sí fallaría — evitar espacios en la ruta si se puede.
