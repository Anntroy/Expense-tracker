...
- **Pendiente de este plan:** fase 3 (PIN opcional al abrir la app, aplicado en el proceso principal).

## Ver por persona (fase 2)

- **Pestaña del mes:** si hay miembros, aparece la tarjeta `MemberBreakdown` ("Por persona") con ingresos, gastos y balance de cada miembro del mes (sin contar los desactivados; los archivados y "Sin asignar" solo salen si tienen movimientos). Esa tabla **siempre compara a todos**. El selector "Ver" de la tarjeta filtra el resto de la vista (cifras, donut y lista) a una persona, con un aviso "Mostrando solo: …" y "Quitar filtro". El filtro se hace en el cliente (`matchesPerson`) sobre los movimientos ya cargados del mes. Si se agrega un movimiento que quedaría oculto por el filtro, el filtro se quita para que se vea (mismo criterio que el cambio de mes al agregar).
